...
def main():
    ...
    # Global attributes
    meta_f = out_loc + 'metadata/flux_metadata_level%s_%smin.xlsx'%(level,avp)
    meta = pd.read_excel(meta_f)

    var_f_estimates = out_loc + '/specific_variables/flux-estimates-level%s.xlsx'%level
    var_estimates = pd.read_excel(var_f_estimates)

    var_f_components = out_loc + '/specific_variables/flux-components-level%s.xlsx'%level
